...
rel_phase = pi*sin(n_rel*pi*z_grid/(condensate_length.*1e6));
com_phase = pi*sin(n_com*pi*z_grid/(condensate_length.*1e6));
...
fitfun_wc = fittype(@(a,x) a*cos((n_rel*pi/l).*x).*cos((n_com*pi/l).*x));
fitfun_woc = fittype(@(a,x) a*sin((n_rel*pi/l).*x).*(cos((n_rel*pi/l).*x)).^2);
