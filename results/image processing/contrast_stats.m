...
avg_contrast_woc_stats = zeros(length(t_tof), num_samples);
avg_contrast_wc_stats = zeros(length(t_tof), num_samples);
avg_contrast_woc_stats_ref = zeros(length(t_tof), num_samples);
avg_contrast_wc_stats_ref = zeros(length(t_tof), num_samples);
...
for i = 1:length(t_tof)
    ...
    for j = 1:num_samples
        ...
        coarse_z_grid = linspace(-condensate_length/2, condensate_length/2, coarse_z_res);
        ...
        avg_contrast_woc_stats(i,j) = trapz(coarse_z_grid, extraction_suite_woc.contrasts)/condensate_length;
        avg_contrast_wc_stats(i,j) = trapz(coarse_z_grid, extraction_suite_wc.contrasts)/condensate_length;
        avg_contrast_woc_stats_ref(i,j) = trapz(coarse_z_grid, extraction_suite_woc_ref.contrasts)/condensate_length;
        avg_contrast_wc_stats_ref(i,j) = trapz(coarse_z_grid, extraction_suite_wc_ref.contrasts)/condensate_length;
...
save('contrast_stats_vs_tof.mat', 'rel_phase','com_phase', 'avg_contrast_woc_stats_ref', ...
    'avg_contrast_wc_stats_ref', 'avg_contrast_wc_stats','avg_contrast_woc_stats','t_tof')