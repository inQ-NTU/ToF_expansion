...
interference_suite = class_interference_pattern(coarse_phase_profile);

%generate rho tof
rho_tof = interference_suite.tof_full_expansion();
rho_tof = interference_suite.convolution2d(rho_tof);
rho_tof = interference_suite.add_gaussian_noise(rho_tof, 0.05);

%initialize phase extraction class
phase_extraction_suite = class_phase_extraction(rho_tof);
