...
interference_suite = class_interference_pattern(phase_profile);

%generate rho tof
rho_tof_fine = interference_suite.tof_full_expansion();
%convolution
rho_tof_conv = interference_suite.convolution2d(rho_tof_fine);
%coarse graining
rho_tof_coarse = imresize(rho_tof_conv, [coarse_dim, 1.2*coarse_dim]);
%add noise
rho_tof_noisy = interference_suite.add_gaussian_noise(rho_tof_coarse, 1/SNR);
...
%%%%%%%%%%%%%Figure 4%%%%%%%%%%%%%%%%%
img1 = figure;
f4 = tight_subplot(1,2,[0.1,0.075],[0.2, 0.2],[0.1, 0.1]);
...
%%%%%%%%%%Figure 5%%%%%%%%%%%%%%%%%%%%%%%%%%
img2 = figure;
f5 = tight_subplot(2,2,[0.1,0.075],[0.15, 0.15],[0.15, 0.15]);
axes(f5(1))
...
axes(f5(2))
imagesc(fine_grid_x,fine_grid_z,rho_tof_conv)
xticks([])
yticks([])
...
cb.Position = cb.Position + [0.1,0,0,0.4];
