...
contrast_woc_stats = zeros(length(t_tof), num_samples);
contrast_wc_stats = zeros(length(t_tof), num_samples);
contrast_woc_stats_ref = zeros(length(t_tof), num_samples);
contrast_wc_stats_ref = zeros(length(t_tof), num_samples);
...
for i = 1:length(t_tof)
    ...
    for j = 1:num_samples
        ...
        %phase extraction
        extraction_suite_woc = class_phase_extraction(img_rho_tof_woc', t_tof(i), flag_width);
        extraction_suite_wc = class_phase_extraction(img_rho_tof_wc', t_tof(i), flag_width);

        extraction_suite_woc_ref = class_phase_extraction(rho_tof_full_woc, t_tof(i),flag_width);
        extraction_suite_wc_ref = class_phase_extraction(rho_tof_full_wc, t_tof(i), flag_width);
        ...
        contrast_woc = sum(extraction_suite_woc.contrasts)/length(extraction_suite_woc.contrasts);
        contrast_wc = sum(extraction_suite_wc.contrasts)/length(extraction_suite_wc.contrasts);
        contrast_woc_ref = sum(extraction_suite_woc_ref.contrasts)/length(extraction_suite_woc_ref.contrasts);
        contrast_wc_ref = sum(extraction_suite_wc_ref.contrasts)/length(extraction_suite_wc_ref.contrasts);
        
        contrast_woc_stats(i,j) = contrast_woc;
        contrast_wc_stats(i,j) = contrast_wc;
        contrast_woc_stats_ref(i,j)= contrast_woc_ref;
        contrast_wc_stats_ref(i,j) = contrast_wc_ref;
...
save('contrast_stats_vs_tof.mat', 'rel_phase','com_phase', 'contrast_woc_stats_ref', ...
    'contrast_wc_stats_ref', 'contrast_wc_stats','contrast_woc_stats','t_tof')