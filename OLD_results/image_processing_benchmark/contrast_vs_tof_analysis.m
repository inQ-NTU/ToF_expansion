close all
load('contrast_stats_vs_tof.mat')
...
for i = 1:length(t_tof)
    median_woc(i) = median(contrast_woc_stats(i,:));
    median_wc(i) = median(contrast_wc_stats(i,:));
    median_woc_ref(i) = median(contrast_woc_stats_ref(i,:));
    median_wc_ref(i) = median(contrast_wc_stats_ref(i,:));

    iqr_woc(i) = quantile(contrast_woc_stats(i,:),0.75) - quantile(contrast_woc_stats(i,:),0.25);
    iqr_wc(i)= quantile(contrast_wc_stats(i,:),0.75) - quantile(contrast_wc_stats(i,:),0.25);
    iqr_woc_ref(i) = quantile(contrast_woc_stats_ref(i,:),0.75) - quantile(contrast_woc_stats_ref(i,:),0.25);
    iqr_wc_ref(i) = quantile(contrast_wc_stats_ref(i,:),0.75) - quantile(contrast_wc_stats_ref(i,:),0.25);
end
